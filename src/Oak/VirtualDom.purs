...
renderTag h (Tag name attrs children) =
  let
    rendered = sequence $ map (renderTag h) children
  in
    N.render name (combineAttrs attrs h) rendered
...
concatAttr
  ...
  -> N.NativeAttrs
  -> N.NativeAttrs
concatAttr handler (EventHandler name msg) attrs = N.concatHandlerFun name
  ( \_ ->
      handler msg
  )
  attrs

concatAttr handler (StringEventHandler name f) attrs = N.concatEventTargetValueHandlerFun name
  ( \e ->
      handler (f e)
  )
  attrs
...
concatAttr handler (KeyPressEventHandler name f) attrs = N.concatHandlerFun name
  ( \e ->
      handler (f e)
  )
  attrs
